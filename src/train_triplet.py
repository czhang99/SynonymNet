...
parser.add_argument("--margin", type=float, default=1, help="The margin for the loss function")
...
parser.add_argument("--model_type", type=str, default='confluence', help="""confluence(default) | bilstm | gru
                                                                    confluence: the context encoder
...
parser.add_argument("--max_epochs", type=int, default=5, help="Max epochs to train.")
...
parser.add_argument("--inference_file", type=str, default='knn-triple_contexts.txt', help="Inference file name.")
parser.add_argument("--inference_label_file", type=str, default='knn-siamese_contexts.txt',
                    help="Inference file name.")
parser.add_argument("--k_num", type=int, default=5, help="Number of knn during inference")
arg = parser.parse_args()

logs_path = '../log/train_triple/' + arg.dataset + '/' + arg.run_name
...
inference_outputs = [outputs, loss]
...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=False)) as sess:
    ...
    while True:
        ...
        a_ids_data, a_context_ids_data, a_keyword_index, a_len_data, p_ids_data, p_context_ids_data, p_keyword_index, p_len_data, n_ids_data, n_context_ids_data, n_keyword_index, n_len_data = data_processor.get_batch_triple(
            arg)
        if len(a_ids_data) != 0:
        ...
        if data_processor.end == 1:
            ...
            def valid(full_path, in_vocab):
                ...
                pred_scores = np.concatenate([np.concatenate(pred_scores_pos), np.concatenate(pred_scores_neg)], axis=0)
                true_scores = np.concatenate([np.ones(np.concatenate(pred_scores_pos).shape),
                                              -np.ones(np.concatenate(pred_scores_neg).shape)], axis=0)
                true_labels = np.concatenate([np.ones(np.concatenate(pred_scores_pos).shape),
                                              np.zeros(np.concatenate(pred_scores_neg).shape)], axis=0)
                fpr, tpr, thresholds = sklearn.metrics.roc_curve(true_scores, pred_scores, pos_label=1)
                auc = sklearn.metrics.auc(fpr, tpr)
                map = sklearn.metrics.average_precision_score(true_labels, pred_scores, average='micro')
                df = pd.DataFrame({'model': 'triplet', 'score': pred_scores, 'class': true_scores})
